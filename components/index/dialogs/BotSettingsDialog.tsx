    ...
    <Dialog onClose={() => setIsDialogOpened(false)} open={isDialogOpened} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <Stack direction="row" alignItems="flex-end">
          <TextField label="チャネルアクセストークン" sx={{ flex: 1 }} value={channelAccessToken} onChange={e => setChannelAccessToken(e.target.value)} required/>
      ...
      <DialogActions>
        ...
        loading={isChannelAccessTokenValidating}>{editingBotId.length > 0 ? "編集" : "追加"}</LoadingButton>
      </DialogActions>
    </Dialog>
  );
}
